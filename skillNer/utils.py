...
class Utils:
    ...
    def one_gram_sim(self, text_str, skill_str):
        ...
        def _compute_distance_sim(txt, skill):
            str_distance_similarity = jellyfish.jaro_similarity(
                txt.lower(), skill.lower())
            return str_distance_similarity

        try:
            vec_similarity = token1.similarity(token2) if tokens[0].has_vector and tokens[
                1].has_vector else _compute_distance_sim(text_str, skill_str)
            return vec_similarity
        except Exception as ex:
            return _compute_distance_sim(text_str, skill_str)
    ...
    def compute_w_ratio(self, skill_id, matched_tokens, late_match_penality=0.1):
        ...
        token_ids = sum([(1 - late_match_penality * skill_name.index(token))
                         for token in matched_tokens if token in skill_name])
